...
// The five self questions, then the five ex questions, asked one screen at a time.
const SELF_STEPS = REFERENCE_QUESTIONS.map((q) => `YOU: ${q.text}`);
const EX_STEPS = REFERENCE_QUESTIONS.map((q) => `YOUR EX: ${q.text}`);
...
export function handleUssd(text) {
  ...
  if (choice === '2') {
    return 'END Ex-Files checks who you SAY you are vs. who your ex says you are. #WeLoveNerds';
  }
  ...
  // Flow: parts[1..5] = self answers, parts[6..10] = ex answers.
  const answers = parts.slice(1); // everything after the "1" menu pick
  const totalNeeded = SELF_STEPS.length + EX_STEPS.length; // 10
  ...
  const { score, band, verdict } = redFlagIndex(self, ex);
  const beat = typeBeat(self, ex);
